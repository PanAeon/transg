...
impl Config {
    pub fn get_directories(&self) -> Vec<DirMapping> {
       self.directories.iter().map(|x| DirMapping {
           label: x.to_string(),
...
fn empty_config() -> Config {
 Config {
    ...
    refresh_interval: 3,
    file_manager: ExternalCommand { cmd: "nautilus".to_string(), args: vec!["{location}".to_string()] },
    terminal: ExternalCommand { cmd: "alacritty".to_string(), args: vec!["--working-directory".to_string(), "{location}".to_string()] },
}
}
pub fn get_or_create_config() -> Config {
    ...
    if !config_path.exists() {
        ...
        write(&config_path, cfg).expect(format!("Failed to create {:?}", &config_path).as_str());
    }
