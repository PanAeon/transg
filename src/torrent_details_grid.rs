...
use gtk::{Label, CompositeTemplate};
use crate::objects::TorrentDetailsObject; 
...
impl ObjectImpl for TorrentDetailsGrid {
    ...
    fn properties() -> &'static [ParamSpec] {
            static PROPERTIES: Lazy<Vec<ParamSpec>> = Lazy::new(|| {
                vec![
                  ParamSpecObject::new( "details", "details", "details", TorrentDetailsObject::static_type(), ParamFlags::READWRITE,),
                ]
            });
            PROPERTIES.as_ref()
    }
    ...
    details.property_expression("downloaded-ever")
       .chain_closure::<String>(gtk::glib::closure!(|_: Option<gtk::glib::Object>, i: u64| {
            format_size(i.try_into().unwrap())
        }))
       .bind(&self.downloaded.get(), "label", gtk::Widget::NONE);
    details.property_expression("percent-complete")
       .bind(&self.completed.get(), "label", gtk::Widget::NONE);
    details.property_expression("priority")
       .bind(&self.priority.get(), "label", gtk::Widget::NONE);
    details.property_expression("seed-ratio-limit")
       .bind(&self.ratio_limit.get(), "label", gtk::Widget::NONE);
    details.property_expression("upload-ratio")
       .bind(&self.ratio.get(), "label", gtk::Widget::NONE);
    details.property_expression("rate-upload")
            .chain_closure::<String>(gtk::glib::closure!(|_: Option<gtk::glib::Object>, i: u64| {
                format_download_speed(i.try_into().unwrap())
            }))
       .bind(&self.rate_down.get(), "label", gtk::Widget::NONE);
    
    details.property_expression("size-when-done")
       .chain_closure::<String>(gtk::glib::closure!(|_: Option<gtk::glib::Object>, i: u64| {
            format_size(i.try_into().unwrap())
        }))
