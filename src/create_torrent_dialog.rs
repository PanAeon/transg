...
pub fn add_torrent_dialog2(vbox: &gtk::Box, 
...
pub async fn add_torrent_dialog3(response: gtk::ResponseType, 
...
pub async fn add_torrent_dialog<W: IsA<gtk::Window>>(window: Rc<W>, sender: mpsc::Sender<TorrentCmd>, magnet_url: Option<String>, torrent_file:Option<PathBuf>,  filter:Rc<gtk::CustomFilter>) {
  ...
  add_torrent_dialog2(&vbox, &magnet_url, &torrent_file, _cancel_fetch.clone(), _magnet_done.clone(), magnet_data.clone(), &destination, &start_paused_checkbox);
  let response = dialog.run_future().await;
  dialog.close();
  add_torrent_dialog3(response, 
                             sender, 
                             magnet_url, 
                             torrent_file,
                             filter,
